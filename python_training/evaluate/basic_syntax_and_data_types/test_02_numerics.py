...
class TestUserInput:
    ...
    def test_numeric_comparison_operations(self):
        ...
        a, b = test_scope.get("a"), test_scope.get("b")
        assert all(
            isinstance(var, bool) for var in test_scope.values() if not ismodule(var)
        )
    ...
    def test_numeric_formatting(self):
        ...
        assert (
            test_scope.get("formatted_int") == f"${test_scope.get('currency_int'):,.2f}"
        )
        assert (
            test_scope.get("formatted_float")
            == f"${test_scope.get('currency_float'):,.2f}"
        )
        assert (
            test_scope.get("formatted_long_float")
            == f"${test_scope.get('currency_long_float'):,.2f}"
        )
    ...
    def test_statistics_builtin_module(self):
        ...
        assert test_scope.get("grades_max_multimode") == max(
            statistics.multimode(param_grades)
        )
